...
strLocalTraceFolder = objReadConfig.get('TraceSetting','TraceFolder')
...
def HAAPTraceAnalysing(lstTraceFiles):	#输入参数是一个列表
	...
	objLogFile = open('../../Result_{}.log'.format(objReadConfig.get('GlobalSetting','TimeNow')), 'w')
	...
	for strFileName in lstTraceFiles:
		if (lambda i:i.startswith('Trace_'))(strFileName):
			print(strFileName + '  Analysing ...')
			objLogFile.write('{}  Analysing ...\r'.format(strFileName))
